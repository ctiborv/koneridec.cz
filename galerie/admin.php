...
function convertTime($dformat,$sformat,$ts) {
 ...
 { // BEGIN function Nahoru($spojeni)
    ...
    $res = PrSql($spojeni,$sql);
 	  ...
 	  $sql = "SELECT * FROM galerie where priorita<$priorita order by priorita DESC LIMIT 0,1";
    $res = PrSql($spojeni,$sql);
...
function Dolu($spojeni)
 { // BEGIN function Nahoru($spojeni)
    ...
    $res = PrSql($spojeni,$sql);
 	  $zaznam = mysqli_fetch_array($res);
 	  $priorita=$zaznam["priorita"];
 	  $sql = "SELECT * FROM galerie where priorita>$priorita order by priorita ASC LIMIT 0,1";
    $res = PrSql($spojeni,$sql);
 	  if ($zaznam=mysqli_fetch_array($res)) {
      $idn=$zaznam["id_galerie"];
      $prioritan=$zaznam["priorita"];
 ...
 function SpravaGalerie($spojeni) {
      ...
      <br>
             <table >
              ...
              <th>Přesun</th>
              <th>Editovat</th>
              <th>Přidat fotografie</th>
              <th>Editovat fotografie</th>
              <th>Smazat galerii</th>
              </tr>
        
        <?
          $sql="SELECT * FROM galerie order by priorita";  
          $res = PrSql($spojeni,$sql);
          while ($zaznam = mysqli_fetch_array($res)) {
            ...
            echo "<td align=\"center\"><a href=\"?sekce=nahoru&id=".$zaznam["id_galerie"]."\"><img width=\"20\" src=\"img/sort_asc.png\"></a>";
            echo "<a href=\"?sekce=dolu&id=".$zaznam["id_galerie"]."\"><img width=\"20\" src=\"img/sort_desc.png\"></a></td>";
            
            echo "<td align=\"center\"><a href=\"?sekce=pridatg&id=".$zaznam["id_galerie"]."\"><img src=\"img/b_edit.png\"></a></td>";
           echo "<td align=\"center\"><a href=\"?sekce=pridatf&id=".$zaznam["id_galerie"]."\"><img src=\"img/j_button2_image.png\"></a></td>";
           echo "<td align=\"center\"><a href=\"?sekce=editf&id=".$zaznam["id_galerie"]."\"><img src=\"img/expandall.png\"></a></td>";
             ?>
    ...
    function resizePhoto($vstup,$vystup,$width,$height,$aspectratio,$quality)  {
     ...
     function EditovatF($spojeni)
      ...
      <br>
       <h1>Fotogalerie : <?echo $nazev?></h1>
        ...
        <?
          $id=$_GET["id"];
          $sql="SELECT * FROM foto where id_galerie=$id";  
          $res = PrSql($spojeni,$sql);
          while ($zaznam = mysqli_fetch_array($res)) {
            echo "<tr>";
     ...
     function PridatF($spojeni)
      ...
      { // BEGIN function PridatDokument
          ...
          if ($odeslat=="Uložit") {
                 $sql = "INSERT INTO galerie (nazev,popis,priorita,datumvlozeni) VALUES ('$nazev','$popis',$priorita,NOW())";
                 $res = PrSql($spojeni,$sql );
   	                    ?>
                      <script language="Javascript">
                  ...
                  $id=$_POST["id"];
                  $sql = "UPDATE galerie SET nazev='$nazev', popis='$popis', priorita='$priorita' where id_galerie=$id";
                  $res = PrSql($spojeni,$sql );
   	                    ?>
                      <script language="Javascript">
                        window.alert('Galerie byla úspěšně aktualizována.')
                        window.location="<? echo $_SERVER["PHP_SELF"] ?>";
                      </script>
                    <?
    }
...
<head>
    ...
    <meta charset="utf-8" />
    <title>Galerie Kone ridec</title>
...
<body>
    <div class="page">
       <div id="header">
       <span id="admin">Galerie suvi.cz</span>
        ...
        <div id="footer">
       <span><a href="../admin.php?cmd=zobrazhlavni_Menu">Zpět na administraci hlavního menu</a></span> 
